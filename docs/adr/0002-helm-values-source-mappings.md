...
The recommendation engine can observe a live Deployment or StatefulSet and update a literal workload manifest in Git. Fleet-managed applications frequently render that workload from an external Helm chart, so the writable source is a chart-specific `values.yaml` key such as `replicaCount`, `resources.requests.cpu`, or `login.resources.requests.memory`. The values document has no Kubernetes resource identity, and different chart components can share one file.
...
- `targetRef` continues to identify the live workload used for observation and recommendations.
...
- Git values must semantically match the live workload before planning; replicas use integer comparison and resources use Kubernetes quantity comparison.
